...
export const particleVertex = /* glsl */ `
  ...
  void main() {
    ...
    // Z-Axis flow (sucking-in effect) — bass speeds up the pull
    float speedMult = 1.0 + uBass * 1.5;
    pos.z += uTime * uSpeed * (1.2 + aRandoms.x * 0.4) * speedMult;
    pos.z = mod(pos.z + 60.0, 70.0) - 60.0;

    // Center clearance — bass pushes particles outward, opening the vortex
    float clearance = 5.0 + uBass * 4.0;
    float r = length(pos.xy);
    if (r < clearance) pos.xy = normalize(pos.xy + 0.001) * (clearance + aRandoms.x * 3.0);

    // Liquid water-flow math — mid frequencies amplify the wave amplitude
    float waveAmp = 0.5 + uMid * 1.5;
    ...
    // Bass + treble make particles bloom
    float audioScale = 1.0 + uBass * 0.8 + uTreble * 0.4;
    float baseScale = (vType < 0.5) ? 1.0 : 2.5;
    float scale = baseScale * (0.2 + vDepth * vDepth * vDepth * 15.0) * audioScale;

    vAudioPulse = uBass + uTreble * 0.5;

    // Spin particles along the current — treble adds jitter to rotation
    float angle = pos.z * 0.05 + aRandoms.y * 6.28 + uTreble * 2.0 * aRandoms.x;
    float s = sin(angle);
    float c = cos(angle);
...
export const particleFragment = /* glsl */ `
  ...
  void main() {
    ...
    if (vType < 0.5) {
      ...
      // Audio pulse increases petal visibility — they "flash" with the beat
      texColor.a *= 0.18 + vAudioPulse * 0.25;
    } else {
      ...
      // Audio pulse brightens blob colors — saturation pop on beats
      finalColor *= 1.0 + vAudioPulse * 0.6;
    }
