...
// AUDIO STRATEGY: near-dark at rest (×0.12). Random ~40% flash bright on treble — changes every ~0.4s.
// Different flares take turns being bright. Dim ones become bright, bright ones dim down.
export const flareVertex = /* glsl */ `
  ...
  void main() {
    ...
    // Time-based shuffle: different flares reactive every ~0.4s
    float seed = floor(uTime * 2.5);
    float shuffleRand = fract(sin(vColorMix * 100.0 + seed * 17.313) * 43758.5453);
    vShuffleReactive = smoothstep(0.4, 0.7, shuffleRand);
...
export const flareFragment = /* glsl */ `
  ...
  void main() {
    ...
    // Near-dark at rest. Shuffled subset flashes on treble — changes every ~0.4s.
