...
const backdropFragment = /* glsl */ `
  ...
  void main() {
    ...
    // Very bright, saturated anime nebula colors
    vec3 mint = vec3(0.3, 0.9, 0.6);
    vec3 cyan = vec3(0.1, 1.0, 0.9);
    vec3 edgeDark = vec3(0.01, 0.1, 0.15);

    vec3 color = mix(mint, cyan, n);
    float dist = distance(vUv, vec2(0.5));
    color = mix(color, edgeDark, smoothstep(0.4, 0.8, dist));
...
const particleFragment = /* glsl */ `
  ...
  void main() {
    ...
    if (vType < 0.5) {
      ...
      finalColor = vec3(1.0, 0.2, 0.5); // Bright Anime Pink
      alpha = texColor.a * smoothstep(1.0, 0.8, vDepth);
    } else {
      // MASSIVE Foreground Blobs
      texColor = texture2D(uTexBlob, vUv);
      finalColor = vec3(0.0, 0.0, 0.0); // FORCE PURE BLACK
      alpha = texColor.a; // Do NOT fade out, keep them solid
    }
...
const glowFragment = /* glsl */ `
  ...
  void main() {
    ...
    // Use noise based on angle and time to create a chaotic, boiling edge
    float n = noise(vec2(angle * 4.0, uTime * 2.0));
    float r = baseRadius + (n * 0.08); // Distort the radius organically

    vec3 coreYellow = vec3(1.0, 0.9, 0.2);
    vec3 hotPink = vec3(1.0, 0.1, 0.5);
    
    // Sharp mix to prevent white-out
    vec3 color = mix(coreYellow, hotPink, smoothstep(0.04, 0.08, r)); 
