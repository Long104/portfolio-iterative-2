...
const backdropFragment = /* glsl */ `
  ...
  void main() {
    ...
    vec3 darkPink = vec3(0.08, 0.0, 0.06);   
    vec3 midAura  = vec3(0.15, 0.02, 0.12);  
...
function KiraKiraVortex() {
  ...
  useEffect(() => {
    return () => {
      ...
      [backdropMat, paintMat, flareMat, sunMat, raysMat, bridgeMat, coreMat].forEach((m) => m.dispose());
    };
  }, [starTex, petalTex, blobTex, gradLUT, noiseTex, backdropGeo, paintGeo, flareGeo, backdropMat, paintMat, flareMat, sunMat, raysMat, bridgeMat, coreMat]);
...
export default function Scene() {
  return (
    <div style={{ width: "100vw", height: "100vh", overflow: "hidden", background: "#01314A" }}>
      <Canvas
        frameloop="demand"
